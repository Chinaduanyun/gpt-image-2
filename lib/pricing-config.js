const { MICROS_PER_YUAN } = require('./constants');
...
const modelProfiles = Object.freeze({
  ...
  'gpt-image-2-official': Object.freeze({
    ...
    // 官方渠道 low 档上游成本极低（×10.5 后全档不足 ¥0.30），一刀切下限会把
    // 1k/2k/4k 磨平成同一个价；改为按请求清晰度取阶梯下限，让价格随清晰度走。
    minimumPerImageMicrosByResolution: Object.freeze({
      '1k': readPositiveSafeInteger('PRICING_GPT_IMAGE_2_OFFICIAL_MINIMUM_PER_IMAGE_MICROS_1K', 300000),
      '2k': readPositiveSafeInteger('PRICING_GPT_IMAGE_2_OFFICIAL_MINIMUM_PER_IMAGE_MICROS_2K', 550000),
      '4k': readPositiveSafeInteger('PRICING_GPT_IMAGE_2_OFFICIAL_MINIMUM_PER_IMAGE_MICROS_4K', 850000)
    })
  })
});

// 按请求清晰度解析生效的单张下限；没有阶梯配置或清晰度不在表内时退回平价下限。
function resolveMinimumPerImageMicros(policy, resolution) {
  const value = policy.minimumPerImageMicrosByResolution?.[String(resolution || '').trim().toLowerCase()];
  return Number.isSafeInteger(value) && value > 0 ? value : policy.minimumPerImageMicros;
}
