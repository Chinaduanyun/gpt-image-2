...
function findTaskLocation(data, taskId) {
  const log = data.spendLogs.find((entry) => entry.type !== 'balance_adjustment' && entry.taskId === taskId) || null;
  return log ? { log, parent: null, index: -1 } : null;
}

function findSpendLogByTaskId(data, taskId) {
  return findTaskLocation(data, taskId)?.log || null;
}

function findLogById(data, id) {
  const log = data.spendLogs.find((entry) => entry.id === id) || null;
  return log ? { log, parent: null, index: -1 } : null;
}
...
function findSnapshotLog(data, snapshot) {
  return snapshot.taskId
    ...
    : findLogById(data, snapshot.id)?.log || null;
}
...
function flattenGenerationLogs(logs) {
  return logs;
}
...
async function refreshLogsFromUpstream(data, logs, config, maxCount = 25) {
  ...
  for (const snapshot of flattenGenerationLogs(logs)) {
    ...
    try {
      ...
      if (latestSnapshot?.status === 'completed' && sourceUrls.length && !hasCompleteImageArchive(latestSnapshot, sourceUrls)) {
        ...
        const merged = await withDataStoreMutation((latestData) => {
          const log = findLogById(latestData, snapshot.id)?.log;
