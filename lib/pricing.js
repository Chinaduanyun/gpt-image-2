...
function estimateGenerationCostMicros(payload) {
  ...
  // 单张下限按请求清晰度解析（官方渠道 1k/2k/4k 阶梯，快速版为平价下限），
  // 解析结果作为快照落进日志，结算沿用快照、与当前配置解耦。
  const minimumPerImageMicros = resolveMinimumPerImageMicros(policy, resolution);
  ...
  const minimumChargeMicros = minimumPerImageMicros * n;
