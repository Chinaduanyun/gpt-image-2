...
function usdToMicros(value, totalMultiplier = pricingConfig.totalMultiplier) {
  return Math.round(Number(value || 0) * totalMultiplier * MICROS_PER_USD);
}

function formatMoneyMicros(micros) {
  return `¥${(Number(micros || 0) / MICROS_PER_USD).toFixed(5)}`;
}
...
function parseMoneyToMicros(value) {
  ...
  return Math.round(number * MICROS_PER_USD);
}
...
function sanitizeGenerationPayload(payload, config) {
  ...
  if (clean.output_compression !== undefined) {
    clean.output_compression = Math.max(0, Math.min(100, Number(clean.output_compression) || 0));
  }
