...
function setSessionCookie(res, token) {
  res.setHeader('Set-Cookie', `imageGenToken=${encodeURIComponent(token)}; Max-Age=${Math.floor(SESSION_TTL_MS / 1000)}; Path=/; SameSite=Lax; HttpOnly`);
}

function clearSessionCookie(res) {
  res.setHeader('Set-Cookie', 'imageGenToken=; Max-Age=0; Path=/; SameSite=Lax; HttpOnly');
}
