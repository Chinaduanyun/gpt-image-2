    ...
    ns.els.myLogsList.innerHTML = filtered.map((log) => {
      ...
      const actions = canReuse ? `
        <div class="history-card-actions">
          ...
          ${canDelete ? `<button class="secondary compact danger-button history-delete-btn" type="button" data-history-action="delete" data-log-id="${ns.escapeHtml(log.id)}">隐藏作品</button>` : `<button class="secondary compact history-delete-btn" type="button" disabled title="执行中、未结算或需关注的任务不能隐藏">不可隐藏</button>`}
        </div>` : '';
      return `
